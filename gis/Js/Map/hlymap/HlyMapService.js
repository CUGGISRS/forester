...
define([
    ...
    HlyMapService.prototype.init = function () {
        ...
        this._contMap.scene.on("pointermove", function (e) {
            ...
            that.raiseEvent(eventarg, "KeyPointPointermove");
        });
        ...
        this._contMap.scene.on("pointerclick", function (e) {
            ...
            if (feature && olLayer && olLayer.isThemeLayer) {
                ...
                that.raiseEvent(eventarg, "themeLayerPointerClick");
            }
        });
        ...
        this._contMap.scene.on("pointerclick", function (e) {
            ...
            var olLayer = e.olLayer;
            //已开启巡护区查询，且无选中对象或由选中对象时图层为非专题图层
            if (that._canRaiseXhqClickEvent && (!e.feature||(olLayer && !olLayer.isThemeLayer))) {
                //可以点击巡护区触发事件
                ...
                $.get(that._serviceUrlConfig.getPatrolAreaByLocation, {
                    ...
                    if (data.PatrolAreaInfo) {
                        ...
                        that.raiseEvent(eventarg, "xhqMouseClick");
                    }
                });
            }
        });
     ...
     * 地图清空
     */
    HlyMapService.prototype.mapClear = function () {
        this._themeObjectManager.clearAll();
     ...
     * @param label 标注，是否需要标注
     */
    HlyMapService.prototype.gotoLocation = function (x, y, resolution, label) {
        this._contMap.viewControl.jumpTo([x, y]);
        if (!resolution) {
            resolution = 2.1457672519402802e-5;
        }
        this._hlyMap.setSmartResolution(resolution);
        //标注
        if (label) {
            var lableId = "坐标点图标";
            var labelType = "坐标点图标"
            var existObj = this._featureContainerManager.exist(lableId, labelType);
            if (existObj) {
                //存在则删除
                this._featureContainerManager.removeMapBusinessItem(lableId, labelType);
            }
            ...
            locationFeature.setStyle(new ol.style.Style({
                image: new ol.style.Icon({
                    ...
                    src: baseUtil.getMapIconsUrl(),
                    scale: 1,
                    offset: [146, 0],
                    size: [30, 37],
                })
            }));
            this._featureContainerManager.addFeatureItem(lableId, {}, locationFeature, this._hlyMap.tmpLayerDataSource, labelType, true);
        }
    };
     ...
     * @param show 是否显示
     */
    HlyMapService.prototype.showHxqMap = function (show) {
        this._hlyMap.xhqBaseMapLayer.setVisible(show);
    };
     ...
     * @constructor
     */
    HlyMapService.prototype.poiItemsShow = function (poiItems) {
        //标注并缩放到所有结果
        //先移除已有的
        var labelType = "地名图标集合";
        this._featureContainerManager.removeMapBusinessItemList(labelType);
        //添加新增的
        ...
        for (var i = 0; i < poiItems.length; i++) {
            var poiItem = poiItems[i];
            var cord = [poiItem.LONGITUDE, poiItem.LATITUDE];
            var locationFeature = new ol.Feature({
                geometry: new ol.geom.Point(cord),
            });
            locationFeature.setProperties(poiItem, true);
            locationFeature.setStyle(new ol.style.Style({
                image: new ol.style.Icon({
                    anchor: [.5, 1],//图标位置对应的地图点
                    src: baseUtil.getMapIconsUrl(),
            ...
            this._featureContainerManager.addFeatureItem(i.toString(), poiItem, locationFeature, this._hlyMap.tmpLayerDataSource, labelType, true);
        ...
        if (coordinates.length > 1) {
            geometry = ol.extent.boundingExtent(coordinates);
            var padding = .1 * this._hlyMap.contMap.scene.map2d.getSize()[0];
            var hasJumped = this._hlyMap.setSmartCenter(geometry,false,[padding, padding, padding, padding]);
        } else {
            geometry = coordinates[0];
            var hasJumped = this._hlyMap.setSmartCenter(geometry);
            if (hasJumped) {
                this._hlyMap.setSmartResolution(2.1457672519402802e-5);
            }
        }
    };
    ...
    HlyMapService.prototype.jumpToHly = function (itemId, item) {
        ...
        if (themeObject) {
            //相同当前专题，直接跳出不做处理
            var hasJumped = this._hlyMap.setSmartCenter(themeObject.businessItem.value.feature.getGeometry());
            if (hasJumped) {
                this._hlyMap.setSmartResolution(2.1457672519402802e-5);
            }

            this._hlyMap.hlyLayerDataSource.setSelected(themeObject.businessItem.value.feature.getId());
            return;
        }
        //清除以前的
        ...
        //跳转到该图标并选中
        var hasJumped = this._hlyMap.setSmartCenter(targetFeature.getGeometry());
        if (hasJumped) {
            this._hlyMap.setSmartResolution(2.1457672519402802e-5);
        }
    ...
    HlyMapService.prototype.trackOnHly = function (itemId) {
        ...
        var hasJumped = this._hlyMap.setSmartCenter(this._lockOnFeature.getGeometry());
        if (hasJumped) {
            this._hlyMap.setSmartResolution(2.1457672519402802e-5);
        }
    ...
    HlyMapService.prototype.showGj = function (themeBelongsTo, itemIdBelongsTo, gjItem) {
        ...
        if(!themeBelongsTo&&!itemIdBelongsTo){
            var themeObject = this._getThemmeObject("轨迹", gjItem.LineId);
            if (themeObject) {
                //相同当前专题，直接跳出不做处理
                //跳转到该专题对象
                var padding = .16 * this._hlyMap.contMap.scene.map2d.getSize()[0];
                this._hlyMap.setSmartCenter(themeObject.businessItem.value.singleLine.getExtent(), false, [padding, padding, padding, padding]);
                return;
            }
        }
        //同一主题下显示唯一轨迹，删除以前的轨迹
        ...
        if (themeBelongsTo && itemIdBelongsTo) {
            ...
            //不隶属于任何专题，自己为轨迹专题
            //删除其他专题，创建轨迹专题
            this.closeCurrentGjTheme();
            gjBusinessItem = this._createGjBusinessItem(gjItem);
            this._themeObjectManager.setThemeObject("轨迹", gjBusinessItem);
        }
        //跳转到该轨迹
        var padding = .16 * this._hlyMap.contMap.scene.map2d.getSize()[0];
        this._hlyMap.setSmartCenter(gjBusinessItem.value.singleLine.getExtent(), false, [padding, padding, padding, padding]);
    };
    ...
    HlyMapService.prototype.showXhq = function (themeBelongsTo, itemIdBelongsTo, patrolDetail) {
        //考虑已有专题情况
        if(!themeBelongsTo&&!itemIdBelongsTo) {
            var themeObject = this._getThemmeObject("巡护区", patrolDetail.PatrolAreaInfo.ID);
            if (themeObject) {
                //相同当前专题，直接跳出不做处理
                //跳转到该区域
                var padding = .16 * this._hlyMap.contMap.scene.map2d.getSize()[0];
                this._hlyMap.setSmartCenter(themeObject.businessItem.value.feature.getGeometry().getExtent(), false, [padding, padding, padding, padding]);
                return;
            }        }
        ...
        //跳转到该区域
        var padding = .16 * this._hlyMap.contMap.scene.map2d.getSize()[0];
        this._hlyMap.setSmartCenter(xhqBusinessItem.value.feature.getGeometry().getExtent(), false, [padding, padding, padding, padding]);
    };
     ...
     * 行政区划点击事件支持
     */
    HlyMapService.prototype.enableRaiseXhqClickEvent = function (enable) {
        this._canRaiseXhqClickEvent = enable;
    };
    ...
    //	//		var feature = xhqDatasource.addDataItem(xhqItem, checkTag);
    //	//		feature.isBaseMap = true;
    //	//	}
    ...
    HlyMapService.prototype.showBj = function (themeBelongsTo, itemIdBelongsTo, bjItem) {
        //考虑已有专题情况
        if(!themeBelongsTo&&!itemIdBelongsTo) {
            var themeObject = this._getThemmeObject("报警", bjItem.ID);
            if (themeObject) {
                //相同当前专题，直接跳出不做处理
                var hasJumped = this._hlyMap.setSmartCenter(themeObject.businessItem.value.feature.getGeometry());
                if (hasJumped) {
        ...
        if (!(themeBelongsTo && itemIdBelongsTo)) {
            //自己为专题数据，支持跳到
            var jumped = this._hlyMap.setSmartCenter(bjBusinessItem.value.feature.getGeometry());
            if (jumped) {
                this._hlyMap.setSmartResolution(2.1457672519402802e-5);
            }
        }
    ...
    HlyMapService.prototype.showRd = function (themeBelongsTo, itemIdBelongsTo, rdItem) {
        //考虑已有专题情况
        if(!themeBelongsTo&&!itemIdBelongsTo) {
            var themeObject = this._getThemmeObject("热点", rdItem.HOTSPOT_ID);
            if (themeObject) {
                //相同当前专题，直接跳出不做处理
                var hasJumped = this._hlyMap.setSmartCenter(themeObject.businessItem.value.feature.getGeometry());
                if (hasJumped) {
        ...
        if (!(themeBelongsTo && itemIdBelongsTo)) {
            //自己为专题数据，支持跳到
            var jumped = this._hlyMap.setSmartCenter(rdBusinessItem.value.feature.getGeometry());
            if (jumped) {
                this._hlyMap.setSmartResolution(2.1457672519402802e-5);
            }
        }

...
define([
    ...
    XhqThemeService.prototype.init = function () {
        ...
        this._contMap.layerManager.addLayer(this._baseLayerContainer, true);
        ...
        this._contMap.scene.on("pointerclick", function (e) {
            ...
            if (that._canRaiseXhqClickEvent) {
                ...
                $.get(that._option.getPatrolAreaByLocationUrl, function (data) {
     ...
     * 清除临时状态或图形
     */
    XhqThemeService.prototype.clearTmp = function () {
        //停止播放轨迹、选中
        this.cancelSelected();
        this.trackPlayClose();
    ...
    XhqThemeService.prototype.onSearchAllXhqWithinTown = function (xhqItems, xiangName) {
        ...
        if (!existLayer) {
            //不存在，则清空并添加
            var xhqLayer = this._layerManage.newXhqLayer();
            xhqLayer.layerLevel = "乡级";
            xhqLayer.layerName = xiangName;
            //覆盖风格
            xhqLayer._styleFunction = function (feature) {
                if(feature.style){
